...
| 10 | `payload_enc` só era purgado após `processed_at`; evento travado guardava PII para sempre | purga conta de **`received_at`** e **anula o payload E o `last_error`**, deixando só metadado não sensível | §13.1, §13.3, teste 46 |
...
**Aprovado e inalterado por decisão do dono:** `RETENCAO_PAGAMENTO_DIAS` **sem valor em código** até validação jurídica/contábil — a LGPD não fixa prazo universal, e o prazo é função da finalidade (§13.1). **Desde 2026-09-05 há um valor PROPOSTO** (1825 dias / 5 anos, pelo coordenador), com fundamentação escrita no §13.1; falta validar, não definir.

Fechado e não reaberto: afiliado fora · GA4 e Meta CAPI dentro · sem trial · flag só na venda · crédito monetário **só Pix→Pix** · `ASAAS_MIN_CHARGE_CENTS` sem default · cobertura contígua com tolerância de 120 s · `RETENCAO_PAGAMENTO_DIAS` **sem número em código** até aval jurídico · Asaas na lista nominal de subprocessadores · tarifa de R$ 0,00 como **nota operacional, nunca premissa** · `cpfCnpj` obrigatório e **não persistido**.
...
ga_client_id, fbp, fbc text     -- SÓ no 1b-B, junto de escritor+purga (§14); zerados na exclusão (§13)
...
last_error text                        -- NULL depois da purga (§13.3)
...
| **queda de tier** (agendada, sobreposta ou por defeito) | **consulta, e nunca congela.** `_queda_de_tier` só pergunta "o tier caiu?" — a consulta existe para dar ao reparo a chance de restaurar um grant truncado; se ela não restaurar, **escreve o que os grants dizem**. Só `indisponivel` não escreve. Motivo e a regressão que matou a versão anterior: §4.1.1 D, parágrafo do tier |
...
**A matriz decide DATA, nunca TIER — e a queda de tier NÃO cria célula nova; ela muda quem ENTRA aqui.** `_queda_de_tier` manda à consulta a redução que `_e_reducao` não vê (tier caiu, data igual): sem isso, um grant de tier alto **truncado** por falha de materialização passa direto e a varredura escreve o tier menor sem ninguém confirmar. **Continuam sendo 7 células.**

**Mas o veredito da matriz não pode CONGELAR uma queda de tier**, e é aqui que a v7 corrigiu a v6. Como todo reparo move só o `ends_at` (invariante 2), a matriz é **incapaz de confirmar tier**: "não confirmou" é o caso NORMAL dessa entrada, não a exceção. Tratar `nao_reduz`/`reparo_insuficiente` como "não escreve" mantinha a conta no tier ALTO por tempo indefinido — e o Tester mostrou que isso engolia o **caso comum**, não a exceção: `pro_max [-60d,-30d]` vencido com `essencial [-35d,+30d]` vigente (assinou o barato 5 dias antes de o caro vencer) congelava em `pro_max`. **Verde no `HEAD`, vermelho no branch** — regressão, não teto.

A regra que fica, e que é menor do que a que ela substitui:

1. tier caiu → **consulta** (o reparo tem sua chance: se o grant do tier alto estava truncado e a assinatura dele está viva, a célula 7 o estica e o tier volta sozinho — nada é escrito);
2. consulta **falhou** (`indisponivel`) → não escreve, alerta. Sem informação nunca se escreve;
3. veredito na lista de quem ESCREVE — `reduz`, `reparou`, `nao_reduz` (célula 6) → **escreve o que os grants dizem**, e registra `projecao_queda_de_tier`.

**A lista é de quem ESCREVE, nunca de quem segura** (R3-1). A v7 listava quem segura (`== "indisponivel"`) e a **célula 5** ficou de fora: conta `pro_max` com a assinatura VIVA no Stripe e sem grant ativo dela era rebaixada para `essencial`, contra a promessa literal da própria célula. Numa lista de quem segura, o default de uma célula nova é ESCREVER — errado num caminho de dinheiro.

E o que tornava a condição inexprimível era a **colisão de nomes**: as células 5 e 6 devolviam as duas `nao_reduz`, e elas querem respostas OPOSTAS no eixo do tier — na 5 falta informação sobre a assinatura viva (não sabemos o tier dela), na 6 a informação está completa e a queda é real. **A célula 5 passou a ter veredito próprio, `sem_grant`**, que é também o que a célula 7 devolve quando o `esticar_grant` não casa linha nenhuma (mesmo estado de conhecimento). Enquanto dividiram a palavra, nenhum teste conseguia separá-las — e não havia teste de célula 5 **no eixo do TIER**. No eixo da DATA ela já tinha o 9l, e escrever "na suíte inteira" contradizia o próprio §16.

**A contiguidade saiu inteira** — a caminhada de cadeia, a tolerância e a distinção "agendado × sobreposto × defeito". Ela existia só para evitar o congelamento; sem congelamento, ela não tem mais o que decidir, e cada conceito a menos aqui é um lugar a menos para o próximo conserto abrir defeito. O custo é uma chamada ao Stripe por travessia de fronteira de downgrade agendado, que é rara.

**Por que escrever é o erro certo aqui, ao contrário do eixo da DATA.** Os dois erros possíveis se curam no MESMO prazo — o próximo evento daquela assinatura reprojeta com `origem="evento"`, que não passa por nada disto. Escrever a queda cedo demais tira tier de quem paga; congelar dá tier de graça a quem não paga. Simétricos, ambos limitados a um ciclo de cobrança, e o alerta sai nos dois. Isso NÃO vale para o eixo da data: lá o erro é `free`, o usuário perde o produto inteiro, e por isso o congelamento continua sendo a regra (§4.1.1 B, casos 9c/9d).
...
| 5 | `active`/`trialing` | **nenhum** (inclusive `legacy` **revogado**) | `sem_grant` | **não escreve** + alerta, **nos dois eixos** |
...
Três invariantes que a matriz protege, e que valem para qualquer célula nova:

1. **O reparo nunca ressuscita.** Só grant `status='active'` é alvo; `revoked` é decisão registrada, não defasagem, e só um **evento** a desfaz. **Hoje isso é condição de BANCO** — o `and status = 'active'` do `esticar_grant` — e há AINDA um filtro `_ativo()` em Python escolhendo o alvo, que faz o mesmo trabalho. As duas são redundantes: cada uma sozinha é invisível para a suíte, e por isso a cláusula SQL tem teste próprio chamando `esticar_grant` direto. Enquanto o reparo era um `upsert_grant` (que grava `active` e zera `revoked_reason`), dependia de um filtro em Python a três linhas de distância: sem ele o reparo reanimava o `legacy` morto pela supersessão, reintroduzindo o D4 pela porta do reparo — e a guarda de versão não segurava, porque aquele reparo carimbava `now()`. **A regra é a mesma; o mecanismo do dano não existe mais** (ver invariante 3).
...
3. **O reparo NÃO escreve `event_version` — essa coluna é marca d'água de EVENTO, e o reparo não é evento.** Ele estica o grant por `esticar_grant` (`UPDATE` que move só o `ends_at` e carimba `last_event_id = REPARO_EVENT_ID`), nunca pelo `upsert_grant`.

> A v6 deste plano dizia o contrário, e **estava errada**. O reparo carimbava `event_version = epoch(now())` porque era a única forma de passar pela guarda `excluded.event_version > plan_grants.event_version` do upsert, e disso saíram três defeitos:
>
> - **R2-1 (medido).** O remendo daquilo — `or last_event_id = REPARO_EVENT_ID` na reclassificação — **não tem guarda de versão nenhuma**. Depois de um reparo, um `invoice.paid` de **2023** deixava de ser obsoleto e autorizava escrita de acesso. E a janela não são os segundos ao redor do reparo: o marcador só sai quando um evento aplica o upsert, então numa conta congelada ele fica **indefinidamente**.
> - **R2-2 (pré-existente, some junto).** `revoke_grant` exige `event_version <= %s`; com o grant carimbado em `now()`, um `deleted` atrasado não revogava nada.
> - **O argumento que sustentava os dois não se sustenta.** Ele dizia: "o reparo lê o Stripe LIVE em T0, logo todo evento criado antes de T0 descreve fato que o Stripe já refletia". São **dois relógios** — `event.created` é o da Stripe, `T0` é `datetime.now()` do nosso processo, e `created` tem precisão de SEGUNDOS (é a premissa do §6.1) —, e o alvo do reparo pode ser o grant **`legacy`**, que não é o objeto que o `deleted` nomeia. Os dois furos são do Tester; conferi e procedem.

**O `UPDATE` FECHA a célula 7 em vez de abrir** — a objeção que eu tinha levantado contra ele estava errada, e o motivo é que ela julgou a categoria ("mexer na célula 7 é arriscado") em vez de ler o que a mudança faz. `esticar_grant` é **estritamente mais estreito** que o upsert: não pode INSERIR linha que sumiu; não zera `revoked_reason`/`revoked_at` nem regrava `status`, então a invariante 1 vira condição de BANCO (`and status = 'active'`) em vez de precondição em Python a três linhas de distância; não regrava `plan_stored` nem `starts_at`, então a invariante 2 vira estrutural (a coluna não está no `SET`); e `and ends_at < %s` faz "só estica, nunca encurta" ser condição do `UPDATE` em vez de depender de a célula 6 ter cortado antes.

**Consequência declarada:** o reparo deixa de disparar a supersessão do `legacy` (o `upsert_grant` de `source='stripe'` a dispara; o `UPDATE` não). É o certo pelo §5.1 — quem mata o `legacy` é o primeiro EVENTO real do Stripe, não uma passada da varredura. **A direção do erro NÃO é conservadora, e dizer isso era falso:** o Tester construiu um `legacy` ativo convivendo com um `stripe` e a projeção escreveu acesso **270 dias além** do que o Stripe cobra. O que segura em produção é outra coisa — o `RESYNC_LEGACY_GRANTS_SQL` (`db/schema.py:60`) só cria `legacy` para quem **não tem grant nenhum** (`and not exists (select 1 from plan_grants g where g.user_id = a.user_id)`), então `legacy` ativo nunca coexiste com `stripe` ativo. **Se alguém relaxar aquele `not exists`, isto acorda.** Nenhum teste cobre o acoplamento; quem escrever um, escreva contra a regra nova.
...
|---|---|---|
| `upsert_grant` | idempotente, **e o retorno classifica o evento** | `on conflict (source, external_ref)` + guarda de `event_version`. O `returning` vem vazio em situações sem relação umas com as outras, então classificar por "o upsert aplicou?" é errado: a pergunta é **"um evento MAIS NOVO já decidiu este grant?"**, respondida por **comparação de versão** contra a linha existente (`_NAO_E_EVENTO_VELHO`: mesmo `user_id`+`source`+`external_ref`, `status='active'`, `event_version <= entrando`). Devolve o **`id`** na reentrega do 5xx, no **evento IRMÃO do mesmo segundo** (`checkout.session.completed` + `invoice.paid` de compra imediata — o empate do §6.1 é caso real). O reparo **não aparece** nesta lista: ele não escreve `event_version` (invariante 3), então não bloqueia evento nenhum e não precisa de cláusula própria — a que existia abria o R2-1; devolve **`None`** só para o que não se aplica (versão estritamente menor, grant revogado, outro dono, `pix` superado). **É isto que impede o retry do 5xx de ser classificado como evento obsoleto.** Quem consome: `_materializar_assinatura`, que com `None` **não escreve acesso** (nem `set_payment_status`, nem `recompute_entitlement`) e loga `billing_evento_obsoleto`. Os efeitos 3–9 do ramo (comissão, GA4/CAPI, e-mail, funil, `claim_trial_for_user`) **continuam rodando**: obsoleto quer dizer "outro evento já decidiu o ACESSO", não "a fatura não foi paga". **Comparar `last_event_id` com o do evento entrando, em vez da versão, era um defeito**: deixava a conta `trialing` com a fatura paga no empate, e `past_due` com a fatura paga na corrida com a varredura |
...
**A guarda vale para QUALQUER grant `pix` futuro ainda vigente, de qualquer tier** (P1-4, Codex #304). A primeira versão só bloqueava tier igual ou maior, e o resto ficou declarado como teto — errado: com um grant Pix de tier MENOR já pago e futuro, o cliente compra um ano inteiro do tier maior sobrepondo os 365 dias daquele e recebe **crédito zero**. Ganho de tier não paga um ano duplicado.

**DECIDIDO PELO DONO (2026-09-07): a recusa é a resposta, não um interino.** O checkout devolve
**409** com `error = "pix_future_purchase_conflict"`, `plan` e `covered_until`, deixando claro que já
existe uma compra Pix futura incompatível. **Não cobra e não altera grant algum.** A exceção
`CoberturaJaPaga` (`core/services/pix_pricing.py`) já carrega `ERRO`, `plano` e `cobertura_ate` — o
1b-B traduz em uma linha e **não deve reconsultar o banco** para montar a mensagem.

> **Evolução separada — substituição do grant futuro com crédito.** Creditar o grant futuro e
> substituí-lo seria melhor para o cliente, e foi recusado como escopo deste PR: exige **modelar a
> linhagem do crédito** (qual cobrança pagou o quê, e quando uma substitui a outra) e **definir
> estorno e chargeback das duas cobranças** — a original e a substituta. **Não é apenas "um pouco
> mais de código"**: é trabalho próprio, fora do 1b-A e do 1b-B, e o §11 mostra o que custa tratar
> estorno sobre grant encadeado (uma coluna de estado por evento, e o `revoke` decidindo sozinho o
> que fica de pé).
...
Quando `preço − crédito < mínimo`, **não se cobra o mínimo**: a compra vira **agendada** (starts no vencimento, preço cheio, crédito 0), e a tela diz isso antes do pagamento. Sem `credit_forfeited_cents` — caminho inalcançável.
...
drenar_evento(event_id):
    ...
    se cobranca is None:
        ...
        insere linha em pix_unmatched_payments + admin_notify   # NÃO em pix_charges
        marca processed_at; sai
...
| `PAYMENT_REFUNDED` | → `refunded` | `[revoke]` |
| `PAYMENT_PARTIALLY_REFUNDED` | → `refunded_partial` | **nenhum** (só alerta) |
| `PAYMENT_REFUND_IN_PROGRESS` | **nenhuma** | **nenhum** (log) |
| `PAYMENT_REFUND_DENIED` | **nenhuma** | **nenhum** (log) |
| `PAYMENT_CHARGEBACK_REQUESTED` | **nenhuma** | **nenhum** (só alerta) |
| `PAYMENT_CHARGEBACK_DISPUTE` | **nenhuma** | **nenhum** (só alerta) |
| `PAYMENT_AWAITING_CHARGEBACK_REVERSAL` | **nenhuma** | **nenhum** (log) — **é a nossa VITÓRIA** |
...
| `paid` | **no-op** (nenhum efeito, §8.2) | no-op | no-op | `PAYMENT_REFUNDED` → `refunded` + `[revoke]` | `PAYMENT_PARTIALLY_REFUNDED` → `refunded_partial`, não revoga, alerta | **no-op** (não revoga; alerta em REQUESTED/DISPUTE) | confere pago × esperado |
| `paid_orphan` | no-op | no-op | no-op | → `refunded` | → alerta | **no-op** (só alerta) | lista de conciliação |
| `refunded_partial` | no-op | no-op | no-op | → **`refunded`** + `[revoke]` | no-op (já parcial) | **no-op** (só alerta) | confere devolvido × esperado |
| `refunded`/`chargeback` | só por reconciliação manual | no-op | no-op | no-op | no-op | no-op | log |

> **A coluna CHARGEBACK não transiciona mais, e a mudança é do 1b-B** (decisão do dono, pendência 2 do §17.1). O curinga `PAYMENT_CHARGEBACK_*` saiu: são **três eventos nomeados** — `PAYMENT_CHARGEBACK_REQUESTED`, `PAYMENT_CHARGEBACK_DISPUTE` e `PAYMENT_AWAITING_CHARGEBACK_REVERSAL` —, **nenhum** muda status e **nenhum** revoga. Abrir disputa não é perder disputa, e o `AWAITING_CHARGEBACK_REVERSAL` é a nossa VITÓRIA: sob o curinga, ele revogava o acesso do cliente que a gente tinha acabado de ganhar. Quem revoga quando perdemos é o `PAYMENT_REFUNDED` que chega depois, pela coluna REFUND total. Os dois primeiros disparam `admin_notify`; nome novo cai em desconhecido. O estado `chargeback` continua existindo no DDL — hoje só se chega nele por reconciliação manual.
...
**Correção P1-2 (Codex #304): ele NÃO é um estado de `pix_charges`, e não podia ser.** `plan`, `plan_stored`, `price_cents`, `credit_cents`, `amount_cents` e `public_token` são `not null`, e **nenhum desses dados existe num webhook órfão** — a linha era literalmente ininserível. Das duas saídas, a escolhida é **tabela própria, `pix_unmatched_payments`**, e não colunas anuláveis: o registro não é uma cobrança nossa (não tem plano, preço nem dono), e afrouxar seis `not null` tiraria a garantia do caminho de 100% das vendas para acomodar um caso que não é venda — todo leitor futuro de `amount_cents` passaria a precisar de um ramo de NULL.
Ela guarda o que o webhook de fato traz: `asaas_payment_id`, `external_reference`, `value`, `netValue`, `status`, `dateCreated`, `customer`, `received_at`, `notified_at`. **Nasce no 1b-B, junto do dreno que a escreve** — regra do dono (§14). No 1b-A o `check pix_charges_status_valido` já **proíbe** o estado em `pix_charges`, para ninguém tentar enfiá-lo lá enquanto isso.
...
### 13.1 Categorias (parâmetros nomeados em `db/pix_charges.py` — menos o `RETENCAO_OUTBOX_DIAS`, que nasce em `db/webhook_outbox.py` no 1b-B, §13.3)
...
| `paid`/`refunded`/`refunded_partial`/`chargeback`/`paid_orphan`/`orphan_unknown` | **`RETENCAO_PAGAMENTO_DIAS = 1825` (5 anos) — PROPOSTO, PENDENTE de validação** | obrigação legal/fiscal. Proposto pelo **coordenador** em **2026-09-05**, **não** validado por contador ou jurídico; fundamentação no bloco abaixo. Enquanto a validação não vier, o valor efetivo continua sendo o que estiver na env — e com `None` **nada de pagamento é apagado**, que é o lado seguro |
| `pix_webhook_events` | **`RETENCAO_OUTBOX_DIAS = 7` a partir de `received_at`** | reprocessamento e depuração |

**Os 5 anos, e por que 5 — a convergência é o argumento, não cada item isolado.** Três prazos independentes respondem à MESMA pergunta (*por quanto tempo ainda se pode precisar provar o que foi cobrado, de quem e quando*), e os três dão 5 anos:

| origem | dispositivo | prazo |
|---|---|---|
| prescrição civil de cobrança | Código Civil, art. 206, §5º, I — dívida líquida em instrumento particular | 5 anos |
| fiscal | CTN, arts. 173 e 174 — decadência e prescrição tributária | 5 anos |
| consumo | CDC, art. 27 — reparação por fato do serviço | 5 anos |

**Por que não mais:** a LGPD não fixa prazo, fixa **finalidade** (arts. 15 e 16). O art. 16, II ampara guardar para cumprir obrigação legal — que é o gancho dos 5 anos. Acabado o prazo, acaba a finalidade, e guardar além disso deixa de ter base.

**Por que um prazo longo é defensável aqui:** o que fica guardado é **registro financeiro pseudonimizado**, não perfil de pessoa. Depois da exclusão da conta (§13.4 e §12.2) somem o vínculo com o titular, os identificadores de anúncio, o QR e o id do cliente; sobram valores, datas, status e referências. É o que torna o prazo proporcional.

> **Ressalva, e ela não é formalidade:** nem o coordenador nem o Coder é advogado ou contador. O que está acima é **raciocínio, não parecer**. O número é parâmetro nomeado e muda com **uma linha** se a validação apontar outro.

**Duas perguntas ao contador continuam abertas, e as duas podem AUMENTAR o prazo:**
1. **Emitimos nota fiscal dessas cobranças?** Se sim, o prazo de guarda do documento fiscal pode ser maior e passa a mandar.
2. **Há obrigação acessória (SPED, DIRF, ECD) com prazo próprio?** Se houver, alinhar ao **MAIOR** — dois prazos diferentes para o mesmo dado é o pior dos mundos.

**Quando a validação chegar:** confirmar ou corrigir `RETENCAO_PAGAMENTO_DIAS`; a varredura já existente passa a apagar, e a política troca "pelo prazo exigido pela legislação aplicável" pelo prazo. Nada mais muda.

### 13.2 Pseudonimização (molde exato da `plan_trials`)

- `pix_charges.user_id` → **FK `on delete set null` DECLARADA INLINE no `create table`** (`db/schema.py`), mais **`"pix_charges"` em `_USER_FK_SET_NULL_TABLES`** (`db/schema_repairs.py`) e o índice do lado que referencia (`idx_pix_charges_user`).
  **Não existe `ensure_pix_charges_user_fk`, e criá-la seria um defeito.** A `ensure_plan_trials_user_fk` existe porque a `plan_trials` nasceu SEM FK nenhuma, e o `repair_user_fk_cascades` só AJUSTA constraint existente — tabela sem FK passa despercebida. A `pix_charges` nasce COM a FK declarada, então uma função espelhada seria código morto no dia 1.
  **O que a protege é a linha no conjunto**, e ela não é opcional: o `repair_user_fk_cascades` roda em TODO `init_db` e reescreve o `on delete` de toda FK em `users(id)`. Sem `"pix_charges"` lá, ele converte a declaração `set null` em `cascade` na primeira subida, **em silêncio**, e a pseudonimização morre junto com a prova do pagamento. Medido: o log diz `schema_repairs ajustou 1 FK(s): [{'table': 'pix_charges', 'from': 'SET NULL', 'to': 'CASCADE'}]`, e a linha financeira some.
  Duas fontes decidindo o `on delete` da MESMA FK, uma rodando depois da outra a cada boot, é exatamente o que o `CLAUDE.md` §0.7 proíbe.
...
3. **Purga em 7 dias a partir de `received_at`, processado ou não** (nº 10): `update pix_webhook_events set payload_enc = null, last_error = null, purged_at = now() where received_at < now() - %s * interval '1 day' and purged_at is null`. **O `last_error` vira `NULL` na MESMA operação que zera o `payload_enc`, inclusive para evento órfão** (decisão do dono, 2026-09-08). Sobra a linha com `event_id`, `event_type`, `event_version`, `attempts` e `purged_at` — e também `received_at` e `processed_at`, que a purga **não** toca e não pode tocar: `received_at` é o predicado da própria purga e `processed_at` é o do `idx_pix_webhook_pendentes`. Tudo metadado não sensível. **A lista é do que sobra, não uma lista fechada:** asserte o que foi ANULADO, não a ausência das outras colunas. Evento travado perde o reprocesso — aceitável e **já alertado** em `attempts > 5`; o dreno que encontrar `payload_enc is null` marca `processed_at` e sai, sem laço infinito. **A varredura é do 1b-B**, junto do dreno que a usa — no 1b-A ela seria código sem chamador (§0.2).
   > **Copie o SQL como está escrito acima.** O placeholder é `%s` (psycopg), **não** `:dias` — este repositório não usa SQLAlchemy, e um `:dias` copiado daqui vira literal. E a subtração precisa de intervalo: `now() - <inteiro>` é erro de tipo em Postgres (`timestamptz - integer` não existe), então o parâmetro entra como `%s * interval '1 day'` (ou `make_interval(days => %s)`). A versão anterior desta linha tinha os dois defeitos.
   > **`RETENCAO_OUTBOX_DIAS = 7` (§13.1) nasce em `db/webhook_outbox.py`, no 1b-B**, junto da varredura que o consome — não em `db/pix_charges.py`, que é a tabela errada; o cabeçalho da §13.1 já abre a exceção por nome. Hoje ele não existe em `.py` nenhum, de propósito: o 1b-A o deixou de fora por ser constante sem chamador (§0.2), e o cabeçalho de `db/webhook_outbox.py` registra a ausência.
   > **A varredura da outbox NÃO pode filtrar por `user_id`** — `pix_webhook_events` não tem essa coluna (o porquê longo está logo abaixo, em "Limpar na exclusão da conta é INALCANÇÁVEL"). O predicado é só idade + `purged_at is null`. Isto é dito porque a varredura vizinha do §13.2 re-zera `pix_charges` por `user_id is null and purged_at is null`, e as duas são varreduras diárias com `purged_at` no `where`: copiar o predicado de lá para cá dá erro de coluna inexistente na primeira execução.
   > **Perda aceita, e ela não é só da conta excluída:** aos 7 dias o diagnóstico do evento some também para **conta VIVA** — `payload_enc` e `last_error` nulos deixam um evento travado sem forense, e `attempts`/`event_type` é o que sobra para depurar. O dono aceitou isso na #315 em troca de não manter PII por tempo indeterminado; o alerta de `attempts > 5` (§15) é o que precisa chegar ANTES da janela fechar.

**Por que o `last_error` sai junto, e por que nenhuma alternativa serve.** Quem escreve nessa coluna é um filtro de **FORMA** (`_erro_seguro`, `db/webhook_outbox.py`), e forma não separa nome de código: medido em 2026-09-08 (remedir antes de reusar), `'Fulano'`, `'Maria'`, `'João'` e `'joao_silva'` atravessam **inteiros** — são alfanuméricos, nenhum é só-dígitos e nenhum tem corrida de 11+ dígitos. O filtro fecha **documento**; nome ele nunca fechou, e não há forma que o feche. As três saídas consideradas e por que caem:

- **Limpar na exclusão da conta é INALCANÇÁVEL, não é preguiça.** `pix_webhook_events` não tem `user_id`, não tem `charge_id` e não tem FK nenhuma (§3.3, e o `create table` de `db/schema.py`). O único vínculo com o titular mora **dentro do `payload_enc`**, cifrado com Fernet **não-determinístico** — o mesmo texto claro produz ciphertext diferente a cada escrita, então nem igualdade de cifrado serve de índice. Não existe `where` que ache as linhas de um usuário.
- **Decifrar tudo em Python para achar o dono é pior que o problema que resolve.** Cada leitura exige `PiiAccessContext`, então a varredura gravaria **uma linha de `pii_access_log` por evento lido**, com `subject_user_id` de terceiros: registro de acesso a PII forjado em massa, para apagar PII. E o único evento que a busca acharia é o que ainda tem payload — justamente o que a purga por idade já pega.
- **Allowlist de códigos do provedor foi recusada pelo dono.** Ela troca uma perda **certa** de diagnóstico (todo código novo que o Asaas passar a mandar vira `?`) por uma distinção que **continua sem garantia** — nome que colidir com um código da lista passa igual.

E o caso que mais importa é exatamente o que a busca por dono nunca alcançaria: **o evento que trava, e portanto o que mais tem `last_error`, é o órfão** — dado de alguém que **pediu exclusão da conta**, cujo `pix_charges.user_id` **já é `NULL`**. Sobra a idade: aos 7 dias tudo sai, sem precisar saber de quem era.
...
**PR 1b** — 7) `db/schema.py`: `pix_charges`, `pix_webhook_events`, `pix_payment_effects`, índices. **`ga_client_id`, `fbp` e `fbc` nascem no MESMO PR que trouxer seus escritores e a purga do §13.2 — nunca soltas "para já deixar pronto"** (decisão do dono, 2026-09-07; o PR 1b-A criou as outras colunas e deixou estas três de fora justamente por isso). Coluna de rastreio publicitário sem purga é dado que sobrevive à exclusão da conta, e o gap fica descrito por um teste invertido em vez de não existir. `alter table … add column if not exists` é barato num repositório que roda DDL idempotente a cada boot; o gap não é. 8) `db/schema_repairs.py`: **uma linha** — `"pix_charges"` em `_USER_FK_SET_NULL_TABLES`. **NÃO escreva `ensure_pix_charges_user_fk`**: a FK é declarada inline no `create table` (item 7), então a função espelhada da `plan_trials` seria código morto, e teria o efeito colateral de pôr duas fontes decidindo o mesmo `on delete` (§13.2, §0.7). **Feito no PR 1b-A.** 9) `db/pix_charges.py`, `db/webhook_outbox.py` (novos). 10) `core/services/asaas.py` (novo, molde de `core/services/pluggy.py` com `httpx`; erros **sem corpo da resposta**, que traz PII). 11) `frontend/routes/billing_pix.py` (novo). **Ao registrar este router, o portão `tests/test_pix_rota_inerte.py` tem de MUDAR DE FORMA no MESMO PR (feito: virou `tests/test_pix_rota_registrada.py`): ele hoje PROÍBE rota de Pix, e passa a ter de EXIGI-la.** Não é opcional — aquele portão mede o app importado, então um registro atrás de `if os.getenv("PIX_ANUAL_ENABLED")` (a forma que o monólito já usa em `ENABLE_DEV_ENDPOINTS`, `:2062`) passaria verde por ele sem uma linha vermelha, e um portão que segue dizendo "não existe rota de Pix" depois do Pix ligado é mentira verde. Rotas: `POST /billing/pix/checkout`, `GET /billing/pix/{id}` (404 para dono errado), `POST /billing/asaas/webhook` + `EFEITOS_POR_EVENTO`. 12) monólito: `CSRF_EXEMPT_PATHS` +1 (`:1892`), `include_router`, `/billing/subscription` (`:4318`) reconhece Pix **antes** do Stripe (`gateway:"pix"`), `_billing_checkout_for_user` (`:3963`) recusa com Pix vigente, `plans-config` (`:4163`) + `pix_annual_available`, dreno no loop de 60 s. 13) `core/services/engagement_scheduler.py`: reconciliação da saga, cancelamento remoto aos 60 d, retenção/purga/pseudonimização — **são DUAS varreduras com predicados DIFERENTES, e trocá-los quebra: a do §13.2 re-zera `pix_charges` por `user_id is null and purged_at is null`; a da outbox (§13.3) filtra SÓ idade + `purged_at is null` e NÃO pode ter `user_id`, coluna que `pix_webhook_events` não tem** —, e `_check_pix_annual_ending()` — **função irmã** do `_check_trial_ending` (`:206`), não reuso (aquela filtra `plan='pro' and last_payment_status='trialing'`); SQL próprio com `join plan_grants … source='pix'`, janelas 6,5–7,5 e 2,5–3,5 dias, dedup `recent_event_exists(…, 2.0)`. 14) `core/services/email_service.py`: `send_pix_annual_ending_email`. 15) `db/privacy.py` (export + comentário) e `frontend/privacy.html`.
...
**Classificação do evento (§4.1.2) — `tests/test_billing_grants_evento_obsoleto.py`**, e o reparo × evento (invariante 3) em **`tests/test_billing_grants_reparo_nao_e_evento.py`** (9v, R2-1, R2-2: assunto próprio, mesmo estado inicial e mesmo negativo)

> Arquivo próprio porque errar a classificação custa nos DOIS sentidos: largo demais deixa conta paga em `trialing`/`past_due`; estreito demais devolve `active` a conta cancelada e ressuscita grant revogado.

9p. **[negativo]** Ordem fora de sequência em três POSTs: `invoice.paid` (`created` T), `subscription.deleted` (T+200), `invoice.paid` **de novo com T+100** (mais VELHO) → 200, `plan == "free"`, **`last_payment_status == "canceled"`**, grant `revoked`. *Negativo: apague o `if aplicou is None: return False` do `_materializar_assinatura` → vermelho.* **Das três asserções só a do `last_payment_status` DISCRIMINA** — as outras duas passam com e sem o conserto, porque o grant já está protegido pela guarda de versão; está escrito na docstring do teste.
9q. **[positivo — retry do 5xx ≠ evento velho]** `db.set_payment_status` levanta na PRIMEIRA chamada (a falha entra **depois** do grant, que já commitou) → 5xx com `auth_accounts` intacto; o **MESMO** evento reentregue → 200, `plan == "pro"`, validade futura, status `active`, **um** grant. *Negativo: classifique por `last_event_id` igual em vez de versão → vermelho.* O **9b não cobre isto**: lá a 1ª entrega conclui e já escreveu tudo.
9u. **[empate do §6.1, achado do Tester]** `checkout.session.completed` e `invoice.paid` com o **MESMO `created`** e **ids DIFERENTES** (compra imediata) → o segundo **não** é evento velho: a conta termina `active`, não `trialing`. *Negativo: volte a exigir `last_event_id` igual → vermelho.* O `test_trial_lifecycle_completo` era cego a isto porque as fixtures omitiam `id`/`created`, o que deixava os dois eventos com `last_event_id` NULO; **as fixtures foram corrigidas** e ele passou a exercitar o empate — o mesmo defeito de fixture já corrigido no `test_subscription_deleted_volta_pra_free`.
9v. **[corrida com a varredura, achado do Tester]** Reparo da célula 7 **não** carimba `event_version` (invariante 3), então um `invoice.paid` legítimo criado antes da passada da varredura aplica normalmente — **data e tier**, não só o status. *Negativo: faça o reparo voltar a escrever pelo `upsert_grant` com `epoch(now())` → vermelho.*
R2-1. **[a largura do buraco, achado do Tester r2]** O MESMO cenário do 9v com `created = 1_700_000_000` (2023): este **tem** de ser obsoleto. Sem ele, 5 segundos e 3 anos passam igual e o `9v` fica cego — foi assim que a cláusula `or last_event_id = REPARO_EVENT_ID` entrou verde. *Negativo: reponha a cláusula → vermelho.*
R2-2. **[o `deleted` atrasado, some junto com a invariante 3]** Grant reparado pela varredura + `subscription.deleted` com `created` ANTERIOR à passada → o grant **é revogado** e a conta vai para `free`. Enquanto o reparo carimbava `epoch(now())`, o `event_version <= %s` do `revoke_grant` descartava esse `deleted` em silêncio e a conta cancelada voltava a `pro` no `recompute` do próprio ramo do cancelamento. *Negativo: o mesmo do 9v — faça a célula 7 reparar por `upsert_grant` com `epoch(now())` → vermelho.* **Atenção à fixture:** o `created` tem de ser ANTERIOR ao reparo (`int(agora.timestamp()) - 5`); um `created` no futuro (`1_800_000_000` é 2027) passa pela guarda de qualquer jeito e deixa o teste cego ao defeito que ele mede.
9w. **[positivo da opção (B)]** Evento obsoleto DE VERDADE (grant com versão muito maior) → o status **não** é rebaixado, **e** `claim_trial_for_user`, funil e `mark_plan_selected` **rodam assim mesmo**. Sem ele, um conserto que abortasse o ramo furaria a trava de um trial por telefone na vida.
9x. **[posição da supersessão do `legacy`]** REENTREGA (mesma versão, mesmo `event_id`) com `legacy` de versão MAIOR → o `legacy` continua `active`. *Negativo: mova a supersessão para depois da reclassificação → vermelho.* **O caso 14 NÃO cobre isto** (medido: com o caso 14 a supersessão passa nas duas posições), porque ele usa evento velho, que devolve `None` de qualquer jeito.
9y. **[`status = 'active'` da reclassificação]** Empate do §6.1 + `deleted` sem `event["id"]` → `revoke_grant` preserva o `last_event_id` pelo `coalesce` e carimba a MESMA versão, então o grant revogado casa ref, versão e evento; só o `status` o separa. Reentrega do `paid` → **`None`**. *Negativo: tire a cláusula → o grant ressuscita e a invariante 1 do §4.1.1 D cai pela porta dos fundos.*
9z. **[isolamento, CLAUDE.md §0]** Upsert do dono **B** com a `external_ref` de **A** → `None`, e `list_grants(B)` vazia. *Negativo: tire `user_id = %s` da reclassificação → ela devolve o **id do grant de A** para B.* É a única query nova que LÊ `plan_grants`.

**Queda de tier na varredura (§4.1.1 B/D) — `tests/test_billing_grants_tier.py`**

9r. **[a razão de a queda de tier entrar na matriz, parametrizado `pro_max→essencial` e `pro_max→pro`]** Grant `pro_max` **ativo TRUNCADO** (venceu há 10d com a assinatura viva) + grant de tier menor de **outra `external_ref`** vigente há 60d; conta `pro_max`/futuro. A projeção diz o tier menor mantendo a MESMA data, então `_e_reducao` não vê nada. O Stripe devolve a assinatura do `pro_max` viva e mais longa → célula 7 estica o grant e **o tier volta sozinho**, sem ninguém escrever tier. *Negativo: tire o `or _queda_de_tier(...)` de `recompute_entitlement` → escreve o tier menor sem nem consultar, vermelho nos dois parâmetros.*
R2-5. **[o caso COMUM, achado do Tester r2 — a queda de tier ESCREVE]** `pro_max [-60d,-30d]` e `essencial [-35d,+30d]`: o usuário assinou o barato 5 dias antes de o caro vencer, então os grants ficam **SOBREPOSTOS**. O Stripe responde e não confirma tier (não tem como) → veredito `nao_reduz` → **escreve `essencial`** e registra `projecao_queda_de_tier`. *Negativo 1: volte o congelamento da v6 (`nao_reduz`/`reparo_insuficiente` → `None`) → vermelho.* *Negativo 2: tire o `_observar` → vermelho na asserção do registro.* *Negativo 3: faça a célula 6 devolver outro veredito → vermelho.* A asserção é sobre o **veredito dentro da mensagem**, não sobre o tipo do evento: células com o mesmo observável só se separam por ele, e foi ao trocar a asserção do veredito pela do tipo do evento que a célula 5 ficou sem medição nenhuma. Era **verde no `HEAD` e vermelho no branch**: regressão, não teto.
R2-5b. **[o POSITIVO do par]** O MESMO cenário com `FakeStripeSubs(erro=True)` → `indisponivel`: **não escreve**, alerta 1x, e a asserção é sobre o **motivo** (`indisponivel`), porque células diferentes têm o mesmo observável. Sem ele o grupo passaria num código que escreve queda de tier sempre, inclusive com o gateway fora do ar. *Negativo: faça `indisponivel` deixar de segurar a escrita → vermelho.*
R3-1. **[célula 5 no eixo do TIER, achado do Tester r3 — não existia teste dela NESTE eixo; no da data é o 9l]** Conta `pro_max`/+30d, único grant vigente é `essencial`, e o Stripe devolve a assinatura **`pro_max` VIVA** até +60d (o grant dela nunca materializou) → **não escreve**, alerta, e a asserção é sobre o **veredito** (`sem_grant`). *Negativo 1: ponha `sem_grant` na lista de quem escreve → vermelho.* *Negativo 2: faça a célula 5 devolver `nao_reduz` de novo → vermelho, porque volta a colidir com a 6.*
B1. **[o canal de alerta, achado do Manager — a bateria de tier nunca cobria `reduz`]** Redução CONFIRMADA para `free` (célula 3: `FakeStripeSubs(ativa=None)`) → escreve `free/None` e **não** registra `projecao_queda_de_tier`, **nem** alerta. Como `_tier_do_stored("free") == 0`, toda redução para `free` também é queda de tier, e o registro disparava nela com uma mensagem que se contradiz: *"tier caiu de pro para free (veredito reduz); a matriz nao confirma tier"*. Não é raro — é a célula 3 (dunning esgotado) e a célula 1 (conta sem `stripe_customer_id`, ou seja **toda conta Pix do PR 1b**). O dado saía certo; quebrava o CANAL, que é 1x/dia por conta e é o mesmo que deve fazer um humano olhar `indisponivel`/`sem_grant`. *Negativo: tire o `and not _e_reducao(...)` da condição do registro → vermelho.* **O veredito é fixado pela CONSTRUÇÃO** (`ativa=None` é a célula 3 e só ela), não por substring; e onde este arquivo casa veredito, casa na **forma fechada** (`veredito nao_reduz)`, `(sem_grant)`) — `"reduz" in msg` casaria com `"nao_reduz"`.
R3-5. **[o veredito não pode mentir]** `esticar_grant` devolvendo `False` (grant sumiu, foi revogado, ou já esticado entre o `list_grants` e o `UPDATE`) → veredito `sem_grant`, **não** `reparou`: não escreve, e o registro não diz "reparou". *Negativo: ignore o bool (`esticar_grant(...); return "reparou"`) → vermelho.*
9t-b. **[o caminho `reparou` com a queda persistindo]** O alvo do reparo é o grant do tier **MENOR** (é ele que tem a assinatura viva): a célula 7 estica a data, o grant volta vigente e o tier continua caído. `_e_reducao` diz `False` porque a data AUMENTOU → **escreve `essencial`**. Era aqui que a v6 congelava por `reparo_insuficiente`.

> **Apagados nesta rodada, e por quê.** `9s` (downgrade contíguo escreve sem consultar), `9s-b` (cadeia de três) e `9t` (o teto: célula 6 congela no tier alto) sumiram junto com a regra de contiguidade. Os três afirmavam o CONTRÁRIO do contrato novo — sob ele, queda de tier sempre consulta, e só `indisponivel` segura. Ficam nomeados aqui porque teste apagado sem registro é a próxima pessoa reintroduzindo a regra achando que faltava cobertura.
...
9l. **[célula 5, a outra que estava errada]** Assinatura **`active`** no Stripe, `legacy` **revogado** e nenhum outro grant ativo → veredito **`sem_grant`** (era `nao_reduz` até a célula 5 ganhar veredito próprio; ver §4.1.1 D): **não escreve**, alerta, e o `legacy` **continua `revoked`** (nem `status`, nem `revoked_reason`, nem `ends_at` mudam). *Negativo: **as DUAS guardas** têm de cair juntas — o filtro `_ativo()` em Python (`billing_access.py`) **e** o `and status = 'active'` do `esticar_grant`. Elas são redundantes entre si, e desligar uma só deixa a suíte inteira VERDE (medido: 31 verdes em cada variante; com as duas, este 9l fica vermelho). E o vermelho é o `ends_at` do grant revogado ter sido MOVIDO — **não** ressurreição na projeção: `projetar_grants` filtra `status == 'active'`, então esticar um revogado nunca o faz projetar.* Quem mede a cláusula SQL **sozinha** é o `test_esticar_grant_so_estica_grant_ATIVO_do_DONO_e_nunca_encurta` (`tests/test_billing_grants_reparo_nao_e_evento.py`), que chama `esticar_grant` direto — sem a segunda guarda no caminho. **O negativo anterior a este afirmava duas coisas falsas** (que reproduzia com uma guarda só, e que a consequência era ressurreição na projeção); receita de negativo que sai verde é pior que ausência de negativo, porque quem a roda conclui que a invariante está protegida.
...
46. **[REVISTO — nº 10]** Outbox: `payload_enc` sem campos fora da lista minimizada e ilegível sem a chave; **evento travado tem o payload anulado aos 7 dias** e a linha permanece com `event_id`/`event_type`/`attempts` — e com `received_at` e `processed_at`, que a purga não toca (§13.3); asserte as colunas ANULADAS, não uma lista fechada do que sobrou —, **`last_error` também nulo**; dreno com `payload_enc is null` marca `processed_at` e sai. *Negativo 1: conte a purga de `processed_at` → PII de conta excluída retida indefinidamente. **Negativo 2 (o que discrimina a cláusula nova): tire `last_error = null` do `update` da purga, deixando `payload_enc = null, purged_at = now()` → o caso 46 fica VERMELHO na asserção de `last_error`.** Sem ele, quem implementar a purga esquecendo `last_error` passa verde nos dois — e `last_error` é a coluna que carrega o nome da classe da exceção e o `code` do Asaas. **Positivo: evento com 3 dias (DENTRO dos 7) sai da mesma varredura com `payload_enc` e `last_error` intactos e `purged_at` nulo** — sem ele o grupo passa numa varredura que zera a tabela inteira, que é pior que o bug.*
...
**Isto não é solução, é a lista do que está aberto.** As cinco primeiras nasceram do ataque ao PR 1b-A (#304), a **6ª** da revisão de 2026-09-08, e **nenhuma se resolve nele**: o 1b-A entrega tabelas, índices, FK e o contrato de `transicionar`; as cinco primeiras decidem o comportamento do **dreno**, que é código do 1b-B, e a 6ª é uma guarda que só o dreno torna alcançável. Quatro rodadas de redesenho escolheram quatro respostas diferentes para a mesma pergunta e as quatro foram reprovadas — por isso aqui elas ficam registradas como pergunta, não como matriz nova. **A matriz do §11 e o §8.2 continuam como estão.**
...
**2. O curinga `PAYMENT_CHARGEBACK_*` invertia o sinal do evento — FECHADA no 1b-B.**
*Cenário (histórico — esta linha NÃO existe mais no §8.2):* `| PAYMENT_CHARGEBACK_* | → chargeback | [revoke] |` casava **três** eventos com significados diferentes. Pela doc oficial do Asaas: `PAYMENT_CHARGEBACK_REQUESTED` é o chargeback **aberto**; `PAYMENT_AWAITING_CHARGEBACK_REVERSAL` significa que **NÓS GANHAMOS** a disputa (depois dele o status do pagamento volta a `RECEIVED`); e, **perdendo**, o que chega é `PAYMENT_REFUNDED`. Sob o curinga, o evento que anuncia a **vitória** era o que **revogava o acesso do cliente**.
*Evidência:* leitura da doc oficial do Asaas (dono, 2026-09-08). **NÃO verificado contra o Sandbox** — ver §18.
*Decisão do dono (2026-09-08), aplicada no 1b-B:* o curinga saiu. São **três eventos nomeados** em `core/services/pix_drain.py`, **nenhum transiciona e nenhum revoga** — `PAYMENT_CHARGEBACK_REQUESTED`, `PAYMENT_CHARGEBACK_DISPUTE` (os dois em `_ALERTAM`) e `PAYMENT_AWAITING_CHARGEBACK_REVERSAL` (só log). Quem revoga quando perdemos é o `PAYMENT_REFUNDED` que chega depois. **Nome novo cai em desconhecido**, que é o lado seguro: evento não roteado não move dinheiro. O §8.2 e a matriz do §11 já estão nessa forma.

*O que continua aberto:* os **nomes** e o status resultante seguem vindo da doc oficial e **não** foram verificados contra o Sandbox — ver §18. Se um dos três nomes estiver errado, o evento cai em desconhecido e vira log, nunca revogação indevida.
...
**5. Entre `efeito_registrado` e `registrar_efeito` cabe o TRABALHO EXTERNO, não só a linha.**
*Cenário:* dois eventos **distintos** do mesmo `payment.id` (um `PAYMENT_CONFIRMED` e um `PAYMENT_RECEIVED`, ou uma reentrega com `event_id` novo) são drenados **em paralelo**. Cada um trava uma linha **de outbox diferente**, então o `select … for update skip locked` do dreno **não os serializa** — ele serializa por `event_id`, e a chave do efeito é `(asaas_payment_id, effect)`. Os dois leem `efeito_registrado(...) is False`, os dois executam: **dois e-mails ao cliente, dois `purchase` no GA4, dois `Purchase` na CAPI** em cima do mesmo dinheiro. O `on conflict do nothing` de `registrar_efeito` só desempata **depois**, devolvendo `False` a uma das passadas para trabalho que já aconteceu. Ele garante **uma linha por par**, e nada sobre quantas vezes o mundo lá fora foi tocado.
*Evidência:* apontamento do Codex no #304 (2026-09-08), no `registrar_efeito`. Achado de forma independente pelo nosso Tester na mesma rodada, que o classificou como **P3** — a divergência de severidade também é do dono: P3 vira P1 se o efeito `email` ou o `capi` for o que roda duas vezes.
*Por que não se resolve aqui:* **não há dreno.** `grep 'registrar_efeito(' --include=*.py` fora de `tests/` devolve zero (medido em 2026-09-08 — remeça antes de reusar), e serialização sem consumidor não se mede: um lock escrito hoje passaria verde sem provar nada, que é o mesmo motivo pelo qual o `skip locked` ficou fora do 1b-A. O que o 1b-A entrega é o **contrato**, escrito nos docstrings de `registrar_efeito` e `efeito_registrado` (`db/webhook_outbox.py`): o par consulta+insert protege a LINHA, e **serializar por `(asaas_payment_id, effect)` é obrigação do dreno**.
*Direção sugerida pelo Codex, como **opção a avaliar** e não como decisão tomada:* **advisory lock** por `(payment_id, effect)` segurando consulta + execução + registro no mesmo escopo, mais **chave de idempotência do provedor** (GA4/CAPI/e-mail) para o caso do crash entre a execução e o registro — que nenhum lock cobre, porque o processo morre com o lock. Alternativas não descartadas: registrar o efeito **antes** de executar e tratar a falha como retryable (troca "executa duas vezes" por "pode não executar"), ou aceitar a duplicata onde ela for barata e proteger só `email` e `capi`. Qual vale é decisão do dono, no 1b-B.

**6. `registrar_falha` REPÕE `last_error` numa linha já purgada (guarda de PII).**
*Cenário:* corrida. A passada que já leu o `payload_enc` está executando os efeitos quando a varredura do §13.3 carimba `purged_at` e zera `payload_enc`/`last_error`; um efeito falha, `registrar_falha` grava `last_error` de novo, e a varredura — que filtra `purged_at is null` — nunca revisita aquela linha: PII permanente. Passada NOVA não produz o caso, porque sem payload o dreno carimba `processed_at` e sai (§13.3), sem falhar.
*Evidência:* o UPDATE de `registrar_falha` (`db/webhook_outbox.py`) não tem `and purged_at is null`, guarda que o vizinho `marcar_processado` tem. O caminho está escrito por inteiro no docstring de `registrar_falha`, no bloco `TETO CONHECIDO` — é a fonte, isto aqui é o índice.
*Por que não se resolve aqui:* mesma razão da 5 — `registrar_falha` não tem chamador de produção, então a guarda passaria verde sem que a corrida tivesse como ser encenada. A guarda e o teste que a prova são critério do #316.
...
**Os nomes de evento de chargeback vieram da DOC OFICIAL e NÃO foram verificados contra o Sandbox** (§17.1, pendência 2): `PAYMENT_CHARGEBACK_REQUESTED` (disputa aberta), `PAYMENT_CHARGEBACK_DISPUTE`, `PAYMENT_AWAITING_CHARGEBACK_REVERSAL` (**nós ganhamos**; o status volta a `RECEIVED`) e `PAYMENT_REFUNDED` (nós perdemos). **O curinga já saiu** (1b-B): os três primeiros estão nomeados no dreno e nenhum revoga, então o desenho não revoga mais no evento da vitória. **O que falta verificar é a GRAFIA dos nomes**, numa cobrança de Sandbox. Errar um nome hoje custa um log em vez de uma revogação indevida — mas custa também o alerta que deveria ter saído. Silêncio sobre isto se leria como verificado (`CLAUDE.md` §7).
...
1. ~~**`ASAAS_MIN_CHARGE_CENTS`** medido no Sandbox~~ — **FECHADA em 2026-09-05: 500 (R$ 5,00)**, medida contra o Sandbox e gravada no `.env` da raiz. O mínimo incide sobre o LÍQUIDO (§7). Sem a env a venda continua 503, e o caso 54 continua valendo.
2. **`RETENCAO_PAGAMENTO_DIAS`: VALIDAR os 1825 dias (5 anos) propostos** — não mais "definir o prazo". Proposto pelo coordenador em 2026-09-05, com a fundamentação (Código Civil 206 §5º I, CTN 173/174, CDC 27) e a ressalva de que é raciocínio e não parecer, no §13.1. O 1b deixa de estar bloqueado por ausência de número e passa a ter um default explicável. **Duas perguntas ao contador podem AUMENTÁ-LO**: nota fiscal dessas cobranças, e obrigação acessória (SPED/DIRF/ECD) — se houver, alinhar ao maior. Enquanto a env for `None`, nada de pagamento é apagado e a política diz "pelo prazo exigido pela legislação aplicável", sem número.
