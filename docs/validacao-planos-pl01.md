...
| P1b | Depois de pôr a conta em Essencial pelo admin (P6, estado S1), abrir `/app`: a conta nova tem `onboarding_completed_at` vazio e `/app` e `/home` redirecionam para `/onboarding` (`shared.gate_onboarding`). Tocar **"Pular tudo e ir para o app"**. Não escolher opção de resumo nem conectar banco no wizard: o passo de resumo grava as preferências diário/semanal/mensal ("Não quero" desliga o semanal que B5 e E3 precisam encontrar ligado, e ele nasce ligado por padrão), e o banco pertence à seção F | dono |
