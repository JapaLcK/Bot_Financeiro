...
function tipFor(p: Point, simOn: boolean) {
  const rows: TipRow[] = [];
  if (!p.real && p.lo != null && p.hi != null) rows.push({ label: "Faixa provável", value: `${money(p.lo)} – ${money(p.hi)}` });
  if (simOn && p.sim != null) rows.push({ label: "Com a simulação", value: money(p.sim), color: "var(--gain)" });
...
export function TrajectoryChart({ traj, simOn, highlight, drawKey }: {
  ...
  const gainArea = simPath ? line([...future.map((p, i) => [x(today + i), y(p.value)] as [number, number]), ...[...simPts].reverse()]) + "Z" : "";
  ...
  const pick = (clientX: number) => {
    ...
    return Math.max(0, Math.min(n - 1, Math.round(((clientX - r.left - M.l) / iw) * (n - 1))));
  };
  const onMove = (e: PointerEvent) => {
    const i = pick(e.clientX);
    setHover(i);
    showTip(e.clientX, e.clientY, tipFor(pts[i], simOn));
  };
  ...
  const onKey = (e: KeyboardEvent) => {
    ...
    setHover(i);
    const r = box.current!.getBoundingClientRect();
    showTip(r.left + x(i), r.top + y(pts[i].value), tipFor(pts[i], simOn));
  };
    ...
    <div className="chart" ref={box}>
      {width > 0 && (
        <svg width={width} height={height} role="img" aria-label={label} tabIndex={0}
          ...
          {ticks.map((t) => (
            <g key={t}>
              <line x1={M.l} x2={width - M.r} y1={y(t)} y2={y(t)} className={t === 0 ? "grid zero" : "grid"} />
              <text x={M.l - 10} y={y(t)} dy="0.32em" textAnchor="end" className="axis">{axisMoney(t)}</text>
            </g>
          ))}
          {xTicks.map(({ p, i }) => (
          ...
          <g clipPath={`url(#${clipId})`}>
            ...
            {gainArea && <path d={gainArea} className="gain-area" />}
            {fcPath && <path d={fcPath} className="line forecast" />}
            {simPath && <path d={simPath} className="line sim" />}
          ...
          {cur && (
            <g className="cross">
              ...
              {simOn && cur.sim != null && !cur.real && <circle cx={x(hiIndex)} cy={y(cur.sim)} r={4.5} className="sim" />}
            </g>
          )}
        </svg>
      )}
