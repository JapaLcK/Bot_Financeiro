import { useContext, useEffect, useMemo, useRef, useState, type FocusEvent, type ReactNode } from "react";
import { ActionsContext, type Actions } from "../lib/actions";
import { apply, set as setGlobal } from "../lib/store.js";
...
// lugar. O foco do teclado num controle escondido pelo corte também abre.
function Compact({ id, children }: { id: string; children: ReactNode }) {
...
export function LiveAnswer({ blocks, s0, page }: { blocks: Block[]; s0: DashState; page: Path }) {
  ...
  const nodes = blocks.map((b) => b(s));
  if (ledger) nodes.push(<Ledger s={s} />);
  return (
    <ActionsContext.Provider value={actions}>
      {nodes.map((n, i) => n && <Compact key={i} id={`${scope}b${i}`}>{n}</Compact>)}
      <a className="link msg-open" href={href(page)} onClick={() => toPanel(page)}>Abrir no painel<i className="ph ph-arrow-right" aria-hidden="true" /></a>
