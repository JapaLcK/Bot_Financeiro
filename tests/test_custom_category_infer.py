...
def test_history_list_categoria_filtra_sem_depender_de_acento(pro_user_id):
    db.add_launch_and_update_balance(
        pro_user_id, "despesa", 120, "consulta", None, categoria="saúde",
    )
    db.add_launch_and_update_balance(
        pro_user_id, "despesa", 80, "mercado", None, categoria="mercado",
    )

    result = db.list_history(pro_user_id, categoria="saude", page=1, limit=50)

    assert result["total"] == 1
    assert result["items"][0]["categoria"] == "saúde"
    assert result["items"][0]["valor"] == 120
