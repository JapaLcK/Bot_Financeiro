...
# ── issue #444/#446: conexão TERMINAL não pode mandar o dashboard recarregar ─

def _conexao_terminal(status: str) -> dict:
    return {"id": 1, "user_id": 4242, "status": status, "provider": "pluggy"}
...
@pytest.mark.parametrize("status,reason", [
    ("PAUSED", "connection_paused"), ("DELETED", "connection_deleted")])
def test_webhook_atrasado_de_conexao_terminal_nao_faz_broadcast(status, reason, monkeypatch):
    """CONTROLE NEGATIVO: tirar o filtro de `reason` no broadcast (voltar a
    checar só `if uid`) deixa este teste vermelho — o `sync_pluggy_item` REAL
    (não mockado) já devolve `user_id` nesses dois `reason`s por causa do
    filtro de isolamento do #446."""
    monkeypatch.setattr(ps, "get_open_finance_connection_by_item_id",
                        lambda item_id, provider="pluggy": _conexao_terminal(status))
    monkeypatch.setattr(of_routes, "log_system_event", _noop_log)
    disparos: list = []

    async def _broadcast(*a, **kw):
        disparos.append((a, kw))

    monkeypatch.setattr(dashboard.manager, "broadcast_to_user", _broadcast)

    asyncio.run(of_routes._run_pluggy_sync_bg("item-terminal"))

    assert disparos == [], f"{reason} não pode mandar o dashboard recarregar"


def test_broadcast_continua_para_reason_nao_terminal(monkeypatch):
    """CONTROLE POSITIVO: um `reason` que não é terminal (ex.: `item_missing`)
    continua mandando o broadcast como hoje."""
    monkeypatch.setattr(of_routes, "sync_pluggy_item",
                        lambda item_id: {"ok": False, "reason": "item_missing",
                                         "item_id": item_id, "user_id": 4242})
    monkeypatch.setattr(of_routes, "log_system_event", _noop_log)
    disparos: list = []
