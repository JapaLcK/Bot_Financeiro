/**
 * Os dois menus da barra pública (`frontend/nav-auth.js`) não ficam abertos ao
 * mesmo tempo, e ativar uma âncora do menu não deixa o foco órfão.
 *
 ...
 * cada um rodado patchando o `nav-auth.js` e reabrindo a página — menu aberto,
 * Enter em `#funcionalidades`, 390×780, reduced-motion:
 *
 ...
 * `index.html` (medido nas 12 páginas que carregam o `nav-auth.js`); aparelho
