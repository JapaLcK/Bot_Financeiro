...
def so_este_usuario(user_id, monkeypatch):
    ...
    O `PLANS_V2_ENABLED=1` repete o `tests/conftest.py`, que já roda a suíte no
    v2, e o segura se o padrão da suíte mudar: no v1 a varredura sai no
    primeiro `if` e TODO teste daqui passaria
