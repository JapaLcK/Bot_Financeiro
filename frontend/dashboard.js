...
// ══════════════════════════════════════════════════════════════════════
// Wizard de Setup (primeira vez + reconfigurar)
// 4 steps: Boas-vindas → Saldo inicial → Cartões → Pronto
// ══════════════════════════════════════════════════════════════════════

let _wizardState = { step: 1, balance: null, openedManually: false };

function _ensureWizardOverlay() {
  if (document.getElementById("wizard-overlay")) return;
  const html = `
    <div class="overlay" id="wizard-overlay" data-wizard>
      <div class="modal wizard-modal">
        ...
        <div class="wizard-step" data-step="1">
          ...
          <div class="modal-acts" style="margin-top:22px;display:flex;justify-content:space-between;align-items:center">
            <button type="button" class="btn-cancel" onclick="wizardSkip()">Pular tudo</button>
            <button type="button" class="btn-save" onclick="wizardNext()">Vamos lá →</button>
          </div>
        </div>
        ...
        <div class="wizard-step" data-step="2" style="display:none">
          ...
          <div class="invest-form">
            <div class="field">
              <label for="wizard-balance">Saldo atual (R$)</label>
              <input type="number" id="wizard-balance" min="0" step="0.01" placeholder="0,00" autofocus />
            </div>
          </div>
          <div class="modal-acts" style="margin-top:18px;display:flex;justify-content:space-between">
            <button type="button" class="btn-cancel" onclick="wizardBack()">← Voltar</button>
            <div style="display:flex;gap:8px">
              <button type="button" class="btn-cancel" onclick="wizardSkipStep()">Pular</button>
              <button type="button" class="btn-save" onclick="wizardSaveBalance()">Salvar e continuar →</button>
            </div>
          </div>
        </div>
        ...
        <div class="wizard-step" data-step="3" style="display:none">
          ...
          <div id="wizard-cards-list" style="margin-bottom:14px"></div>
          <button type="button" class="mock-cta" style="width:100%" onclick="wizardOpenCardModal()">+ Adicionar cartão</button>
          <div class="modal-acts" style="margin-top:18px;display:flex;justify-content:space-between">
            <button type="button" class="btn-cancel" onclick="wizardBack()">← Voltar</button>
            <div style="display:flex;gap:8px">
              <button type="button" class="btn-cancel" onclick="wizardSkipStep()">Pular</button>
              <button type="button" class="btn-save" onclick="wizardNext()">Continuar →</button>
            </div>
          </div>
        </div>
        ...
        <div class="wizard-step" data-step="4" style="display:none">
          <div style="text-align:center;padding:8px 0 4px">
            ...
            <p class="msub" style="max-width:380px;margin:0 auto">
              ...
              <span style="font-size:.8rem;color:var(--text-3)">Quer reconfigurar depois? Use o menu da sua conta.</span>
            </p>
          </div>
          <div class="modal-acts" style="margin-top:22px;justify-content:center">
            <button type="button" class="btn-save" onclick="wizardFinish()">Começar a usar →</button>
          </div>
        </div>
      </div>
    </div>
...
function _setWizardStep(step) {
  ...
  document.querySelectorAll("#wizard-overlay .wizard-step").forEach(el => {
    el.style.display = parseInt(el.dataset.step, 10) === step ? "" : "none";
  });
  document.querySelectorAll("#wizard-progress .wizard-dot").forEach(el => {
...
function _wizardRenderCardsList() {
  const wrap = document.getElementById("wizard-cards-list");
  if (!wrap) return;
  const cards = (lastData && lastData.credit_cards) || [];
  if (!cards.length) {
    wrap.innerHTML = `<div style="padding:14px;text-align:center;color:var(--text-3);font-size:.85rem;background:rgba(255,255,255,.03);border:1px dashed var(--glass-border);border-radius:10px">Nenhum cartão ainda</div>`;
    return;
  }
...
async function openWizard(opts = {}) {
  _ensureWizardOverlay();
  _wizardState = { step: 1, balance: null, openedManually: !!opts.manual };
  _setWizardStep(1);
  _wizardRenderCardsList();
...
function wizardNext() {
  if (_wizardState.step === 3) _wizardRenderCardsList();
  _setWizardStep(Math.min(4, _wizardState.step + 1));
  if (_wizardState.step === 3) _wizardRenderCardsList();
}
function wizardBack() {
  _setWizardStep(Math.max(1, _wizardState.step - 1));
}
function wizardSkipStep() {
  wizardNext();
}
async function wizardSkip() {
  // Pular tudo: salva timestamp pra não auto-abrir de novo nos próximos 7 dias.
  try { localStorage.setItem("pigbank_wizard_skipped_at", String(Date.now())); } catch(_) {}
  closeWizard();
  showToast("Você pode reconfigurar depois no menu da sua conta.");
}
...
async function wizardSaveBalance() {
  const v = parseFloat(document.getElementById("wizard-balance").value);
  if (isNaN(v) || v < 0) {
    await alertModal("Digite um valor válido (zero ou positivo).", { title: "Valor inválido" });
    return;
  }
  try {
    const resp = await fetch(`${API}/account/${USER_ID}/initial-balance`, {
      method: "POST",
      credentials: "same-origin",
      headers: csrfHeaders({ "Content-Type": "application/json" }),
    ...
    if (!resp.ok) {
      ...
      try { detail = JSON.parse(txt).detail || txt; } catch(_) {}
      if (resp.status === 409) {
        // Conta já tem saldo. Não bloqueia o wizard — pula direto.
        await alertModal(
          "Sua conta já tem saldo. Pra ajustar o saldo, use Conta → Ajustar saldo (em breve).",
          { title: "Saldo já definido" },
        );
        wizardNext();
        return;
      }
    ...
    _wizardState.balance = v;
    showToast(`✓ Saldo inicial: ${_fmtBRL(v)}`);
    wizardNext();
  } catch (err) {
    await alertModal(String(err.message || err), { title: "Erro" });
  }
...
function wizardOpenCardModal() {
  // Reusa modal existente. Quando salvar, re-renderiza a lista do wizard.
  openCardEditModal();
  // Hook: depois que cadastrar cartão, sendRefresh atualiza lastData. Vamos
  // observar via setTimeout pra recarregar a lista no wizard.
  const checkInterval = setInterval(() => {
    const modal = document.getElementById("card-edit-overlay");
    if (modal && !modal.classList.contains("open")) {
      clearInterval(checkInterval);
      // Aguardar o lastData atualizar
      setTimeout(_wizardRenderCardsList, 800);
    }
  }, 300);
}

async function maybeOpenWizardOnLoad() {
  if (!USER_ID) return;
  try {
    const resp = await fetch(`${API}/account/${USER_ID}/setup-status`, {
      credentials: "same-origin",
      headers: csrfHeaders(),
    });
    if (!resp.ok) return;
    const data = await resp.json();
    // Só auto-abre se conta virgem E usuário não pulou recentemente.
    const skipped = localStorage.getItem("pigbank_wizard_skipped_at");
    const skippedRecently = skipped && (Date.now() - parseInt(skipped, 10) < 7 * 24 * 60 * 60 * 1000);
    if (data.is_virgin && !skippedRecently) {
      openWizard();
    }
  } catch (_) {}
}
...
function openRecurringEditModal(rec) {
  ...
  document.getElementById("recurring-variable").checked = isEdit ? !!rec.variable_amount : false;
...
async function saveRecurring() {
  ...
  const payload = {
    ...
    variable_amount: document.getElementById("recurring-mode").value === "manual"
      && document.getElementById("recurring-variable").checked,
...
async function payBill(billId, estimate, name, variavel) {
  ...
  // Valor variável (água/luz): abre o campo vazio pra digitar o valor real;
  // valor fixo: já pré-preenche com o estimado.
  const pergunta = variavel
    ? `Quanto veio a conta de "${name}" este mês? (R$)`
    : `Quanto você pagou de "${name}"? (R$)`;
  const prefill = variavel ? "" : Number(estimate || 0).toFixed(2);
