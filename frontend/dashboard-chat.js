...
(function () {
  ...
  function asksAboutPortfolio(text) {
    const directText = text.replace(/^(?:(?:me\s+)?mostr\w*|quero\s+ver)\s+/i, '');
    ...
    const operation = /\b(?:aport\w*|aplic\w*|deposit\w*|resgat\w*|sac\w*|compr\w*|vend\w*|transfer\w*|moviment\w*|retir\w*|contribui\w*|hist[oó]ric\w*|extrato|invist\w*|investir|investi|rendiment\w*|rendeu|rentabilid\w*|retorn\w*|performanc\w*|desempenh\w*|lucr\w*|valoriza[cç][aã]o)\b/i;
    return !operation.test(text) && subject.test(text)
      && (financialContext.test(text) || barePortfolio.test(directText))
      && (directList.test(directText) || (personal.test(text) && statusRequest.test(text)));
  }
