...
async def _get_current_admin(creds: HTTPAuthorizationCredentials = Depends(_bearer)) -> str:
...
async def _fetch_admin_overview(days: int = 30) -> dict[str, Any]:
...
async def admin_auth_login(request: Request, body: AdminLoginBody):
    ...
    token = _make_admin_jwt(ADMIN_DASHBOARD_USERNAME)
    await log_system_event(
        "info",
        "admin_login_success",
...
async def admin_api_overview(days: int = 30, username: str = Depends(_get_current_admin)):
    data = await _fetch_admin_overview(days=days)
...
async def serve_admin_dashboard():
    return FileResponse(HERE / "admin-dashboard.html")


@app.get("/admin/login")
async def serve_admin_login():
    return FileResponse(HERE / "admin-login.html")
