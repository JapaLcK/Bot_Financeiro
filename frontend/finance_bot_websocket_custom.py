    ...
    async with await db_connect() as conn:
        async with conn.cursor() as cur:
            ...
            investments = await cur.fetchall()
                        ...
                        # Total launches for the requested month (excluindo movimentações internas)
            await cur.execute(
                """
            ...
            # Launches for the requested month (paginated) — exclui movimentações internas (criar/deletar bolsos, etc.)
            await cur.execute(
                """
