...
async def build_xlsx(user_id: int, start_date: date | int, end_date: date | int) -> bytes | None:
    period_start, period_end = _normalize_export_period(start_date, end_date)
    items = await _fetch_export_items(user_id, period_start, period_end)
    if not items:
        return None
    return await asyncio.to_thread(_render_xlsx, items, period_start, period_end)
...
async def build_pdf(user_id: int, start_date: date | int, end_date: date | int) -> bytes | None:
    period_start, period_end = _normalize_export_period(start_date, end_date)
    items = await _fetch_export_items(user_id, period_start, period_end)
    if not items:
        return None
...
async def lifespan(app: FastAPI):
    ...
    async def _wa_periodic():
        try:
            await asyncio.sleep(1)
            from adapters.whatsapp.wa_app import _periodic_report_loop  # noqa: PLC0415
            await _periodic_report_loop()
        except Exception as exc:
            print(f"[wa_periodic] erro: {exc}", file=sys.stderr)
...
async def export_email(
    ...
    xlsx_bytes = await build_xlsx(user_id, period_start, period_end)
    pdf_bytes = await build_pdf(user_id, period_start, period_end)
...
class RecurringIncomeCreatePayload(BaseModel):
    ...
    start_date: str | None = None  # 'YYYY-MM-DD'; default = hoje
    frequency: str = "monthly"     # 'monthly' | 'annual'
    pay_month: int | None = None   # 1-12, obrigatório se annual
...
async def recurring_income_create_route(
    ...
    try:
        item = await asyncio.to_thread(
            ...
            payload.frequency, payload.pay_month,
        )
    except ValueError as exc:
...
async def recurring_income_update_route(
    ...
    try:
        item = await asyncio.to_thread(
            ...
            start_date=payload.start_date, frequency=payload.frequency, pay_month=payload.pay_month,
        )
    except ValueError as exc:
        code = str(exc)
