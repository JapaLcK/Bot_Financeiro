...
- "apaga TODOS os lançamentos" / "apaga tudo" / "limpa meu histórico" / "zera meus lançamentos" / "apaga tudo que registrei" → `delete_all_launches` (DESTRUTIVO EM MASSA — pede confirmação, reverte o saldo). SÓ pra apagar TUDO de uma vez. Pra um lançamento específico use `delete_launch`. NUNCA improvise um "confirma com sim?" pra apagar tudo: CHAME `delete_all_launches` — é ela que registra a confirmação; sem a tool, o "sim" do user se perde.
