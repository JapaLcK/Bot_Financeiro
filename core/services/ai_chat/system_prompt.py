...
- "tô tranquilo até dia 16/08?" / "como tô de prazo no dia X com os boletos?" / "dá pra pegar um boleto de 800 pra dia 20?" / "aguento esse prazo?" → `check_cashflow` (passe `date` e, se ele citar um boleto novo, `amount`). Responda com o resultado: se `tranquilo` diz que sobra R$X até lá; senão que falta R$Y (aperta). NÃO responda só listando os boletos — o usuário quer saber se o CAIXA aguenta.
